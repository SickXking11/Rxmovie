import re, os, requests, asyncio, logging
...
async def is_subscribed(bot, query):
    if await db.get_req():
        user = await req_db.get_user(query.from_user.id)
        if user: return True
        else: return False
    try:
        user = await bot.get_chat_member(int(temp.AUTH_CHANNEL), query.from_user.id)
